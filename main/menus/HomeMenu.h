  ...
  public:
    ...
    void loop() {
      ...
      if (CURRENT_RENDER_TIMESTAMP - RENDER_DELAY > RENDER_TIMESTAMP) {
        ...
        if (dryer.isWorking()) {
          lines[0] = String(dht11.getTemperature()) + "/" + String(configStore.getTemperature()) + "C | " + String(dht11.getHumidity()) + "%";
          lines[1] = formatTime(dryer.elapsedMinutes()) + "/" + formatTime(configStore.getMinutes());
        } else {
          lines[0] = String(dht11.getTemperature()) + "C | " + String(dht11.getHumidity()) + "%";
